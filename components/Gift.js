...
const Gift = (props) => {
    ...
    return <div className="flex flex-col space-y-5 lg:space-y-0 lg:flex-row items-center lg:space-x-5 bg-stone-100 p-5 rounded-xl cursor-pointer" onClick={() =>{
        ...
        {image && <img src={image} width={100} alt={`image ${title}`}/>}
            { max && reserved < max && <div><Image src="/images/plus.svg" height={28} width={28} alt="add" onClick={(e) => {
            e.stopPropagation()
